...
import {
  CprPhysicDocumentModel,
  SectionTemplate,
  Signatory,
} from './cpr-physic-document-model';
...
export class DefaultCprPhysicDocumentModel extends CprPhysicDocumentModel {
  protected sections: SectionTemplate[] = [
    {
    ...
    {
      title: 'EMITENTE',
      contentFn: (cpr) => sanitize(cpr.emitter.qualification),
    },
    {
    ...
    },
    {
      title: '2) DO PRODUTO',
      contentFn: (cpr) =>
        `${cpr.product.name} em Grãos, safra agrícola ${cpr.crop}.`,
    },
    {
      title: '3) DAS CARACTERÍSTICAS DO PRODUTO:',
      contentFn: (cpr) => sanitize(cpr.product.characteristics),
    },
    {
      title: '4) DA QUANTIDADE',
      contentFn: (cpr) =>
        `${cpr.quantityFormatted} Kg equivalentes a ${cpr.sacasFormatted} sacas de 60 kg cada uma.`,
    },
    {
    ...
    },
    {
      ...
      contentFn: (cpr) =>
        cpr.productDevelopmentSite.qualifications.map(
    ...
    },
    {
      ...
      contentFn: (cpr) =>
        `Fica ajustado o valor da CPR em ${cpr.valueFormatted}, referente a quantidade mencionada de ${cpr.product.name}, e acordo com o cronograma de vencimento: ${cpr.paymentScheduleText}.`,
    },
    {
  ...
  constructor(cpr: CprPhysicEntity) {
    super(cpr);
  }
  ...
  public getSignatories(): Signatory[] {
    ...
    signatories.push({
      role: 'Emitente',
      name: this.cpr.emitter.name,
      email: this.cpr.emitter.email,
    });

    if (this.cpr.emitter.individual.spouse) {
      signatories.push({
    ...
    signatories.push({
      role: 'Credora',
      name: this.cpr.creditor.legalName,
      email: this.cpr.creditor.email,
    });

    if (this.cpr.guarantor) {
      signatories.push({
        role: 'Interveniente Garantidor',
        name: this.cpr.guarantor.name,
        email: this.cpr.guarantor.email,
      });
    }
    return signatories;
  }
